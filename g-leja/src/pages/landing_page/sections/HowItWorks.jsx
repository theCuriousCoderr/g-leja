    ...
    <div className="px-16 xs:max-md:px-5 flex flex-wrap justify-between gap-5 w-full">
      ...
      {howItWorks.map((reasons) => {
        return (
          <button
            key={reasons.keyword}
            className="relative group w-[23%] xs:max-md:w-full xs:max-md:mx-auto aspect-square xs:max-md:overflow-scroll outline-orange-500 bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl xs:max-md:rounded-xl overflow-hidden"
          >
            ...
            <div className="hidden xs:max-md:block size-full group-hover:flex group-focus-within:flex items-center justify-center text-sm xs:max-md:text-base transition-all font-inter p-3 text-white font-medium">
              {reasons.text}
            </div>
          </button>
        );
